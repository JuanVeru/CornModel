...
2. **Inferencia en Tiempo Real**: Ejecución optimizada de inferencia en CPU utilizando `tensorflow-cpu` dentro del contenedor backend.
3. **Buzón de Información Dinámica**: La sintomatología, condiciones climáticas favorables y remedios se obtienen dinámicamente desde una base de datos relacional PostgreSQL.
4. **Panel Administrativo Protegido (Admin)**: Módulo seguro de autenticación por JWT para modificar y actualizar la literatura científica de cada clase agrícola mediante un panel de control interactivo.
5. **Carga Automatizada y Semillero (Seeding)**: El sistema inicializa automáticamente las credenciales del administrador y la información por defecto de los tres cultivos en la primera ejecución.
6. **Dockerizado y Modular**: Configuración completa con Docker Compose para desplegar el backend de FastAPI y la base de datos PostgreSQL de forma aislada y limpia.
7. **Interfaz Responsive**: Desarrollada en **Expo (React Native)** compatible para Web, Android, iOS y lista para empaquetarse en un archivo **APK**.
